...
module CspaceConfigUntangler
  ...
  class ConfigFetcher
    ...
    attr_reader :name, :target_dir, :driver, :version, :saved_name

    def get_version
      src = driver.page_source
      src.match(/#{version_matcher_prefix(src)}(.*?)<\/span>/)
        .captures
        .first
        .tr(".", "-")
    end

    def version_matcher_prefix(src)
      return " profile version " if src.match?(" profile version ")
      return " UI version " if src.match?(" UI version ")

      "Release "
    end

    def target_name = "#{name}_#{version}.json"
